...
guard CMIOObjectGetPropertyDataSize(systemObjectID, &deviceListAddress, 0, nil, &dataSize) == noErr
else {
...
guard
  CMIOObjectGetPropertyData(
    systemObjectID, &deviceListAddress, 0, nil, dataSize, &dataUsed, &deviceIDs) == noErr
else {
...
func isAnyCameraRunning(deviceIDs: [CMIOObjectID]) -> (Bool, String?) {
  for deviceID in deviceIDs {
    ...
    let nameStr =
      ...
      ? deviceName!.takeRetainedValue() as String : "(Name not available)"
